...
export const metadata: Metadata = {
  ...
  description: 'Restaurante Marqués en Calle Gran Vía, 45 — Madrid. Horarios de apertura, cómo llegar y teléfono de contacto. Abierto de martes a domingo.',
  openGraph: {
...
const contactInfo = [
  {
    icon: <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#C8DC2E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>,
    label: 'Dirección',
    content: <><span>Calle Gran Vía, 45</span><br /><span>28013 Madrid</span></>,
  },
  {
    icon: <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#C8DC2E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.6 2.18h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6.29 6.29l.91-.91a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>,
    label: 'Teléfono',
    content: <span>+34 91 000 0000</span>,
  },
  {
    icon: <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#C8DC2E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>,
    label: 'Email',
    content: <span>info@marquesmadrid.es</span>,
  },
  {
    icon: <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#C8DC2E" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="1" y="3" width="15" height="13" rx="1"/><path d="M16 8l5 3-5 3V8z"/></svg>,
    label: 'Metro',
    content: <><span>Gran Vía (L1, L5) · 3 min a pie</span><br /><span>Callao (L3, L5) · 5 min a pie</span></>,
  },
];
    ...
    <>
      ...
      <section className="bg-[#172E22] px-5 sm:px-8 lg:px-[52px] py-16 border-b border-[#1E3020]">
        ...
        <p className="anim-fade-up text-[#8AB5A0] text-[17px] leading-[1.7] max-w-[480px]" style={{ animationDelay: '0.25s' }}>
          En el corazón de Madrid, a dos pasos de la Gran Vía. Reserva con antelación, especialmente los fines de semana.
        </p>
      </section>
      ...
      <section className="bg-[#F1EFE9] px-5 sm:px-8 lg:px-[52px] py-16 sm:py-[88px]">
        <div className="grid md:grid-cols-2 gap-16 max-w-5xl mx-auto">
          ...
          <AnimateIn from="left">
            ...
            <SplitText
              text="Cuándo visitarnos"
              as="h2"
              className="font-heading font-bold text-[32px] tracking-[-0.5px] text-[#172E22] mb-8"
              baseDelay={80}
              stagger={55}
            />
            <div className="space-y-0">
              {horarios.map((h, i) => (
                <AnimateIn key={h.dias} delay={i * 60}>
                  <div className={`flex items-baseline justify-between py-4 border-b border-[#D8E2DC] transition-colors duration-200 hover:bg-[#EBE9E3] -mx-2 px-2 rounded-[2px] ${h.cerrado ? 'opacity-40' : ''}`}>
                    <span className="font-heading font-semibold text-[15px] text-[#172E22] min-w-[160px]">{h.dias}</span>
                    <span className={`text-[14px] text-right ${h.cerrado ? 'text-[#5A6B60] italic' : 'text-[#5A6B60]'}`}>{h.horas}</span>
                  </div>
                </AnimateIn>
              ))}
            </div>
            <p className="mt-6 text-[#8A9C90] text-[12px] leading-[1.65]">
              La cocina cierra 30 minutos antes del horario indicado. Recomendamos reservar mesa con antelación.
            </p>
          </AnimateIn>
              ...
              {contactInfo.map((item, i) => (
                <AnimateIn key={item.label} delay={i * 80}>
                  <TiltCard intensity={3}>
                    <div className="flex gap-4 group rounded-[4px] p-3 -m-3 hover:bg-[#E8E5DF] transition-colors duration-200">
      ...
      <section className="bg-[#172E22] px-5 sm:px-8 lg:px-[52px] py-[72px]">
        <AnimateIn className="max-w-5xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-8">
          <div>
            ...
            <p className="text-[#6A9A80] text-[15px] leading-[1.65]">Madrid Centro · 28013<br />A 5 minutos del metro Gran Vía</p>
          </div>
          <MagneticEl>
            <a
            ...
            >
              <svg aria-hidden="true" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
              Ver en Google Maps
            </a>
          </MagneticEl>
        </AnimateIn>
