          ...
          <div className="flex border border-[#C4D5CA] rounded-[3px] overflow-hidden text-[12px] font-bold">
            {(['all', 'interior', 'terraza'] as ZoneFilter[]).map(f => (
              <button
                key={f}
                onClick={() => setFilter(f)}
                className={`px-3 py-[6px] border-r border-[#C4D5CA] last:border-r-0 transition-colors capitalize ${
          ...
          <form onSubmit={handleCreate} className="flex flex-wrap gap-3 items-end">
            ...
            <div>
              ...
              <select value={newLocation} onChange={e => setNewLocation(e.target.value as 'interior' | 'terraza')}
                className="border border-[#C4D5CA] rounded-[3px] px-3 py-2 text-[#172E22] text-sm bg-white outline-none focus:border-[#172E22]">
